...
The dashboard's own fetching is untouched by this section — the tiles keep `range=1mo` and the 30-day sparkline. Separately, `scripts/snapshot.mjs` runs in GitHub Actions (`.github/workflows/daily-commentary.yml`, cron `13 22 * * 1-5` — weekdays, after the last US session close in both summer and winter, off the top of the hour — plus `workflow_dispatch` for manual catch-up) and records the last settled session's closes for later trend analysis and the LLM-written commentary (issue #6). GitHub delays scheduled runs on public repos by minutes to hours (up to 8h observed, issue #8), so nothing below depends on *when* the run happens.
...
- For the six exchange-traded instruments, records the **last settled daily bar**: the bar whose session end — 17:00 New York for the CME/ICE contracts and the dollar index, 15:00 Chicago for the yield indices (`SESSION_CLOSE_HOUR` in `snapshot-lib.mjs`), resolved in the exchange's own zone so daylight time is handled — is at or before the run time, with `ts` set to that session end. `meta.regularMarketPrice` is never recorded: after the futures reopen (22:00 UTC in summer, 23:00 in winter) it is a live print from the *next* session, and even in-window it is a last trade that Yahoo revises overnight, whereas the bar close is the official settlement and matches the year of history the trend statistics run on (§3.7). The tiles keep showing the last trade, so on the COMEX metals the note's level can sit ~1% from the tile's; that gap is accepted. The FX pairs have no settlement and keep the live quote, but if the run drifted past London midnight the quote is swapped for that pair's bar on the session date, so every value on the line describes one session. Bars are dated by the exchange's local calendar date — Yahoo stamps futures bars at local midnight and FX bars at London midnight the day *before*.
- Upserts **one JSON line per session date** — the latest settled date among the exchange-traded instruments, or the run's UTC date if none was fetched — into `public/data/snapshots.jsonl`, sorted by date. A run GitHub delays past midnight still writes the right day; a weekend or holiday run re-records Friday's session (same closes, new `asOf`). Re-running for a session replaces its line, never duplicates it. The file lives in `public/` so Vite ships it with the bundle and it is fetchable from the site's own origin.
...
`prev` is the settled bar before the recorded one (for FX, the bar dated before the quote's London date). Yahoo's `meta.chartPreviousClose` is deliberately **not** used here: it is the close before the *requested range* began (a year earlier for `range=1y`), not the prior day's. The log was reset on 2026-09-06 when the settled-bar rule landed: the earlier lines held last-trade quotes, six of them mid-session prints and one a phantom Sunday, and the merge in §3.7 fills those dates from Yahoo's settled bars instead.

`node scripts/snapshot.mjs --history-out <file>` additionally dumps the full year of `[unix ms, close]` bars per symbol (settled bars only for the exchange-traded ones, timestamped on their session date; plus 365 daily BTC closes from CoinGecko's `market_chart?days=365&interval=daily`, best-effort) together with the session `date`, to a scratch file for the trends step (§3.7); that dump is not committed. The pure parts (`scripts/snapshot-lib.mjs`) are covered by `npm test` (Node's built-in runner, no dependency). The job needs no secrets to run; `MP_PROXY_TOKEN` only improves its odds against Yahoo's datacenter-IP blocking.
...
**Series.** For each of the ten instruments (nine Yahoo keys + `btc`) the series is the snapshot log merged over the fetched history, one bar per UTC date, **the log winning on shared dates** — it is what we observed and committed; the fetch only fills dates we don't own yet. Exchange-traded bars from the log are dated by their `ts` (the session end, §3.6); the FX pairs and BTC spot, which have no session, take the line's session date, so a line written by a delayed run still reads as one day. Every series is then cut at the pack's session date, because the fetched history of a 24-hour instrument can already hold a live bar for the day after.

**Per instrument** (`instruments.<key>`; `null` horizons where history is too short):
- `last`, `lastTs`, `tradedToday` — whether a bar is dated on the pack's session date.
...
**Pack-level:** `date` (the session the pack describes, from the snapshot step's dump; the run's UTC date when built by hand without one), `asOf` (the run time), `tradingDay` (any *exchange-traded* instrument — copper, Brent, the yields, gold, DXY — printed a bar on `date`; FX pairs quote 24/5 and print on Sunday evenings, and BTC never stops, so they don't count. With session dating this is true whenever the snapshot found a session and false only for a hand-built pack against a date with none; the prompt still uses it to avoid narrating a session that didn't happen), `barsPerInstrument` (how much history each stat rests on).
...
**One note per session** (issue #8). Before anything else the script reads the archive; if it already holds a note dated on or after the pack's session date it prints `session <date> already has a note — skipping` and exits 0 without calling the API. Weekend and holiday runs resolve to the last settled session (§3.6), which was written up when it happened, so notes are effectively written for trading days only and those runs cost nothing; a same-session re-run cannot overwrite a note by accident. `--force` — the `regenerate` checkbox on `workflow_dispatch` — regenerates it. The note's `date` is the session date, so a note produced by a run that drifted into Tuesday morning is still Monday's.
...
| Daily commentary job broken (note > 4 days old) | The row reads `no commentary since <date>` and the expanded view flags the last note's date in red; the old text remains readable but is never presented as today's. |
...
5. **Session close hours are a table.** The snapshot's settled-bar rule (§3.6) hardcodes 17:00 New York / 15:00 Chicago per instrument in `SESSION_CLOSE_HOUR`; Yahoo's own `currentTradingPeriod` calls the futures day 04:00–03:59 UTC and is useless for this. If an exchange moves its close, or a contract is added on a different exchange, the table is the one place to update — a wrong hour records the previous day's bar (too early) or a live print (too late).
...
- **Cron drift.** Measured over the first two weeks (issue #8): scheduled runs started 15 minutes to 8 hours after the 21:30 UTC cron, only three of fourteen inside the intended hour. Under the old run-date rule that recorded live evening-session prints as closes, lost two trading days to date collisions, and had a Sunday note narrate a session. The job now never trusts the clock: the snapshot records the last *settled* session and dates its line by it (§3.6), and the note is skipped when that session already has one (§3.8), so a late run only delays the note, never mis-dates it. The cron is `13 22 * * 1-5` — after the futures close in winter (22:00 UTC) as well as summer (21:00), weekdays only, off the top of the hour where GitHub drops queued runs. A dropped run is simply a missing day.
- **Weekends and holidays.** No scheduled run on weekends; a holiday run (or a manual weekend one) re-records Friday's session and writes no note, so the panel shows Friday's read labelled `LAST READ · <date>` until the next session's note lands. No holiday calendar is kept — "was there a session" is read from the data.
- **Catch-up semantics.** `workflow_dispatch` re-runs the *last settled session*: it replaces that session's snapshot line, and skips the note unless `regenerate` is ticked (§3.8). There is no backfill of earlier sessions — a missed day stays a gap in `snapshots.jsonl`; the Yahoo history fetched for the stats pack still covers the gap for trend purposes.
...
- **Cost.** Measured on the first run: 3,370 input + 638 output tokens on `claude-fable-5` ≈ $0.07/run, ≈ $2/month. Over the first twelve notes (to 2026-09-05, `effort: high`) the average was 4.8K input / 1.05K output; weekday-only generation trims about two notes a week. `usage` is stored in every `commentary.json` / `.jsonl` line, so actual spend is auditable from the archive: `jq -s 'map(.usage.inputTokens) | add' public/data/commentary.jsonl`.

...
The note is a static file (`public/data/commentary.json`) written once a day by the CI job described under [Daily snapshot log and commentary](#daily-snapshot-log-and-commentary) — the browser fetches it from the site's own origin. No key, no LLM call, nothing dynamic on the client. If the file isn't there (a fresh clone, a fork that hasn't enabled the job) the row simply doesn't appear. If it's more than three days old the row says `no commentary since <date>` rather than passing off an old note as today's.

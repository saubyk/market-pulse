...
Components use inline styles, **except** anything responsive or theme-state-dependent (like the toggle's icon swap): inline styles outrank media queries, so the ≤640px collapse lives in CSS classes (`.tile-grid`, `.app-shell`, `.tile`) in `src/styles.css`. Put any new breakpoint-dependent styling there, not inline.

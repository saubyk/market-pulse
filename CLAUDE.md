...
Components use inline styles, **except** anything responsive or theme-state-dependent (like the toggle's icon swap): inline styles outrank media queries, so the ≤640px collapse and the ≥1240px wide-desktop layout (sections two-abreast via `.section-grid` so all eight tiles fit one viewport without scrolling) live in CSS classes (`.tile-grid`, `.app-shell`, `.app-frame`, `.section-grid`, `.tile`) in `src/styles.css`. Put any new breakpoint-dependent styling there, not inline.
